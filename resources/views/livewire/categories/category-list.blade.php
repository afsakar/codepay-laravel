    ...
    <div class="w-full">
        ...
        <x-card>
            ...
            <div class="grid grid-cols-2 gap-4 py-4 dark:text-gray-400 dark:bg-gray-800">
                <x-input.text wire:model="filters.search" placeholder="Search Category..."  />
            ...
            <x-table>
                    ...
                    <x-table.row class="text-gray-600 dark:text-gray-400 dark:bg-gray-700 text-center text-sm bg-cool-gray-100">
                        <x-table.cell colspan="5">
                            @unless ($selectAll)
                                <div>
                                <span>
                                    {!! __('You have selected <strong>:selectedCount</strong> items. Do you want to select all <strong>:totalCount</strong> items?', ['selectedCount' => $categories->count(), 'totalCount' => $categories->total()]) !!}
                                </span>
                                    <button wire:click="selectAll" class="text-blue-600 ml-1">{{ __('Select All') }}</button>
                                </div>
                            @else
                                <span>
                            {!! __('You are currently selecting all <strong>:totalCount</strong> items.', ['totalCount' => $categories->total()]) !!}
                        </span>
                            @endif
                        </x-table.cell>
                    </x-table.row>
                ...
                @forelse ($categories as $category)
                    <x-table.row wire:loading.class="opacity-80" class="text-gray-600 dark:text-gray-400 dark:bg-gray-700" wire:key="row-{{ $category->id }}">
                        @permission('categories.delete')
                        ...
                        <x-table.cell>
                            <x-badge :color="$category->type_color">
                                {{ __(App\Models\Category::TYPES[$category->type]) }}
                            </x-badge>
                        </x-table.cell>
                        <x-table.cell>
                            <x-badge :color="$category->status_color">
                                {{ __(App\Models\Category::STATUS[$category->status]) }}
                            </x-badge>
                        </x-table.cell>
                        <x-table.cell title="{{ $category->created_at }}">
                            {{ $category->created_at->diffForHumans() }}
                        </x-table.cell>
                        @permission('categories.update')
                        <x-table.cell>
                            <div class="flex items-center space-x-4 text-sm">
                                <x-button wire:click="edit({{ $category->id }})" aria-label="Edit" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 rounded-lg focus:outline-none focus:shadow-outline-gray text-gray-600 dark:text-gray-400">
