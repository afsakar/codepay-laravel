            ...
            <div class="md:w-1/2 pr-2 space-y-4">
                <x-input.group inline for="filter-status" :label="__('Status')">
                    <x-input.select id="filter-status" wire:model="filters.status">
                        ...
                        @foreach (App\Models\Corporation::STATUS as $key => $value)
        ...
        <x-card>
            ...
            <x-table>
                    ...
                    <x-table.row class="text-gray-600 dark:text-gray-400 dark:bg-gray-700 text-center text-sm bg-cool-gray-100">
                        <x-table.cell colspan="7">
                            @unless ($selectAll)
                                <div>
                                <span>
                                    {!! __('You have selected <strong>:selectedCount</strong> items. Do you want to select all <strong>:totalCount</strong> items?', ['selectedCount' => $corporations->count(), 'totalCount' => $corporations->total()]) !!}
                                </span>
                                    <button wire:click="selectAll" class="text-blue-600 ml-1">{{ __('Select All') }}</button>
                                </div>
                            @else
                                <span>
                            {!! __('You are currently selecting all <strong>:totalCount</strong> items.', ['totalCount' => $corporations->total()]) !!}
                        </span>
                            @endif
                        </x-table.cell>
                    </x-table.row>
                ...
                @forelse ($corporations as $corporation)
                    <x-table.row wire:loading.class="opacity-80" class="text-gray-600 dark:text-gray-400 dark:bg-gray-700" wire:key="row-{{ $corporation->id }}">
                        @permission('corporations.delete')
                        <x-table.cell class="pr-0">
                            <x-input.checkbox wire:model="selected" value="{{ $corporation->id }}" />
                        </x-table.cell>
                        @endpermission
                        <x-table.cell>
                            {{ $corporation->name }}
                        ...
                        <x-table.cell>
                            @if(($corporation->revenue[0]->sum_times_with_exchange_rate) || ($corporation->expense[0]->sum_times_with_exchange_rate))
                                {{ number_format((($corporation->revenue[0]->sum_times_with_exchange_rate) - ($corporation->expense[0]->sum_times_with_exchange_rate)), 2). " ₺" }}
                            @else
                                {{ number_format(0, 2). " ₺" }}
                            @endif
                        </x-table.cell>
                        ...
                        <x-table.cell>
                            <div class="flex items-center space-x-4 text-sm">
                                <x-button wire:click="edit({{ $corporation->id }})" aria-label="Edit" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 rounded-lg focus:outline-none focus:shadow-outline-gray text-gray-600 dark:text-gray-400">
                ...
                <div class="md:grid md:grid-cols-2 md:space-x-4">
                    ...
                    <x-input.group inline for="filter-type" :label="__('Type')" :error="$errors->first('editing.type')">
                        <x-input.select id="filter-type" wire:model.defer="editing.type">
                            ...
                            @foreach (App\Models\Corporation::TYPES as $key => $value)
                                <option value="{{ $key }}">{{ __($value) }}</option>
                            @endforeach
                        </x-input.select>
                    </x-input.group>
