    ...
    <div class="w-full">
                        ...
                        <div class="md:w-1/2 pr-2 space-y-4">
                            <x-input.group inline for="filter-status" label="Status">
                                <x-input.select id="filter-status" wire:model="filters.status">
                                    ...
                                    @foreach (App\Models\Unit::STATUS as $key => $value)
        ...
        <x-card>
            ...
            <div class="grid grid-cols-2 gap-4 py-4 dark:text-gray-400 dark:bg-gray-800">
                <x-input.text wire:model="filters.search" placeholder="Search Unit..."  />
            ...
            <x-table>
                    ...
                    <x-table.row class="text-gray-600 dark:text-gray-400 dark:bg-gray-700 text-center text-sm bg-cool-gray-100">
                        <x-table.cell colspan="5">
                            @unless ($selectAll)
                                <div>
                                <span>
                                    {!! __('You have selected <strong>:selectedCount</strong> items. Do you want to select all <strong>:totalCount</strong> items?', ['selectedCount' => $units->count(), 'totalCount' => $units->total()]) !!}
                                </span>
                                    <button wire:click="selectAll" class="text-blue-600 ml-1">{{ __('Select All') }}</button>
                                </div>
                            @else
                                <span>
                            {!! __('You are currently selecting all <strong>:totalCount</strong> items.', ['totalCount' => $units->total()]) !!}
                        </span>
                            @endif
                        </x-table.cell>
                    </x-table.row>
                ...
                @forelse ($units as $unit)
                    <x-table.row wire:loading.class="opacity-80" class="text-gray-600 dark:text-gray-400 dark:bg-gray-700" wire:key="row-{{ $unit->id }}">
                        @permission('units.delete')
                        <x-table.cell class="pr-0">
                            <x-input.checkbox wire:model="selected" value="{{ $unit->id }}" />
                        </x-table.cell>
                        @endpermission
                        <x-table.cell>
                            {{ $unit->name }}
                        ...
                        <x-table.cell>
                            <x-input.toggle wire:click="toggleSwitch({{$unit->id}})" :active="$unit->status == 'active'" />
                        </x-table.cell>
                        ...
                        <x-table.cell>
                            <div class="flex items-center space-x-4 text-sm">
                                <x-button wire:click="edit({{ $unit->id }})" aria-label="Edit" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 rounded-lg focus:outline-none focus:shadow-outline-gray text-gray-600 dark:text-gray-400">
