...
class BillList extends Component
{
    ...
    public function rules()
    {
        return [
            ...
            'editing.bill_number' => 'required|unique:bills,bill_number,' . $this->editing->id,
    ...
    public function validationAttributes()
    {
        return [
            ...
            'editing.bill_number' => __('Bill Number'),
    ...
    {
        $this->editing = $this->makeBlankBill();
    ...
    {
        $this->useCachedRows();
        if($this->editing->isNot($bill)) $this->editing = $bill;
    ...
    public function create()
    {
        ...
        if($this->editing->getKey()) $this->editing = $this->makeBlankBill();
        $this->editingModal = true;
    }
    ...
    {
        $query = Bill::query()->with('corporation')->with('withholding')->where('company_id', get_company_info()->id)
            ->when($this->filters['status'], fn($query, $status) => $query->where('status', $status))
            ->when($this->filters['search'], fn($query, $search) => $query->where('bill_number', 'like', '%'.$search.'%'))->orderBy('issue_date', 'desc');
    ...
    {
        return view('livewire.purchases.bill-list', [
            'bills' => $this->rows
        ]);
    }
}
